...
void DStatusBarCore::DrawGraphic(FGameTexture* tex, double x, double y, int flags, double Alpha, double boxwidth, double boxheight, double scaleX, double scaleY, ERenderStyle style, PalEntry color, int translation, double clipwidth, double clipheight)
{
	...
	DrawTexture(twod, tex, x, y,
		...
		DTA_ClipLeft, clipwidth > -1 && (flags & DI_ITEM_REVERSECLIP) ? int(x + boxwidth * (1.0 - clipwidth)) : 0,
		DTA_ClipTop, clipheight > -1 && !(flags & DI_ITEM_REVERSECLIP) ? int(y + boxheight * (1.0 - clipheight)) : 0,
		DTA_ClipBottom, clipheight > -1 && (flags & DI_ITEM_REVERSECLIP) ? int(y + boxheight * clipheight) : twod->GetHeight(),
		DTA_ClipRight, clipwidth > -1 && !(flags & DI_ITEM_REVERSECLIP) ? int(x + boxwidth * clipwidth) : twod->GetWidth(),
		DTA_Color, color,
